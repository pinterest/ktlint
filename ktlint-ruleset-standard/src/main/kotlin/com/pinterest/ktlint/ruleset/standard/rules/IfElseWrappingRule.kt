...
public class IfElseWrappingRule :
    ...
    override fun beforeVisitChildNodes(
        ...
        when (node.elementType) {
            THEN, ELSE_KEYWORD, ELSE -> {
                node
                    .outerIf()
                    .let { outerIf ->
                        if (outerIf.textContains('\n') || outerIf.isNestedIf()) {
                            visitBranch(node, autoCorrect, emit)
                        }
                    }
            }
            BLOCK -> {
                node
                    .treeParent
                    ?.takeIf { it.elementType == THEN || it.elementType == ELSE }
                    ?.takeUnless { it.outerIf().textContains('\n') }
                    ?.let {
                        ...
                        emit(
                            it.startOffset,
                            "A single line if-statement should be kept simple. The '${it.elementType}' may not be wrapped in a block.",
                            false,
                        )
                    }
            }
