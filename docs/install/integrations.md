...
* The `ktlintCheck` is bound to the *Gradle check* task. It can also be executed with command `gradle ktlintCheck`.
* The `ktlintFormat` task is not bound to any other task. It can be executed with command `gradle ktlintFormat`.
