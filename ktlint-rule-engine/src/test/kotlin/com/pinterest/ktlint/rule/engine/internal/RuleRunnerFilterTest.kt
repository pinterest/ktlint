...
class RuleRunnerFilterTest {
    ...
    fun `Given that standard rule set is enabled explicitly then run all standard rules except those that are disabled explicitly`() {
        val ruleRunnerFilter = createRuleRunnerFilter(
        ...
        val actual = ruleRunnerFilter.filterAndMapToRuleIds()
    ...
    /**
     * Create a [RuleRunnerFilter] for a given set of [RuleProvider]s and an [EditorConfig].
     */
    private fun createRuleRunnerFilter(
        vararg ruleProviders: RuleProvider,
        editorConfig: EditorConfig,
    ): RuleRunnerFilter =
        RuleRunnerFilter(
            ktLintRuleEngine = KtLintRuleEngine(ruleProviders = ruleProviders.toSet()),
            editorConfig = editorConfig.addPropertiesWithDefaultValueIfMissing(CODE_STYLE_PROPERTY),
        )

    private fun RuleRunnerFilter.filterAndMapToRuleIds() = filter().map { it.ruleId }
