...
object Main {
    ...
    @Option(names = ["--apply-to-idea"], description = ["Update Intellij IDEA settings (global)"])
    private var apply: Boolean = false

    // todo: make it a command in 1.0.0 (it's too late now as we might interfere with valid "lint" patterns)
    @Option(names = ["--apply-to-idea-project"], description = ["Update Intellij IDEA project settings"])
    ...
    @Option(names = ["--install-git-pre-commit-hook"], description = ["Install git hook to automatically check files for style violations on commit"])
    private var installGitPreCommitHook: Boolean = false

    @Option(names = ["--install-git-pre-push-hook"], description = ["Install git hook to automatically check files for style violations before push"])
    private var installGitPrePushHook: Boolean = false

    @Option(names = ["--limit"], description = ["Maximum number of errors to show (default: show all)"])
    private var limit: Int = -1
        get() = if (field < 0) Int.MAX_VALUE else field

    @Option(names = ["--print-ast"], description = ["Print AST (useful when writing/debugging rules)"])
    private var printAST: Boolean = false

    @Option(names = ["--relative"], description = ["Print files relative to the working directory " +
        "(e.g. dir/file.kt instead of /home/user/project/dir/file.kt)"]
    )
    private var relative: Boolean = false

    @Option(names = ["--reporter"], description = ["A reporter to use (built-in: plain (default), plain?group_by_file, json, checkstyle). " +
    ...
    @Option(names = ["--repository"], description = ["An additional Maven repository (Maven Central/JCenter/JitPack are active by default) " +
        "(value format: <id>=<url>)"]
    )
    ...
    @Option(names = ["--repository-update", "-U"], description = ["Check remote repositories for updated snapshots"])
    ...
    @Option(names = ["--ruleset", "-R"], description = ["A path to a JAR file containing additional ruleset(s) or a " +
        "<groupId>:<artifactId>:<version> triple pointing to a remote artifact (in which case ktlint will first " +
        "check local cache (~/.m2/repository) and then, if not found, attempt downloading it from " +
        "Maven Central/JCenter/JitPack/user-provided repository)"]
    )
    ...
    @Option(names = ["--stdin"], description = ["Read file from stdin"])
    ...
    @Option(names = ["--help", "-h"], help = true, hidden = true)
    private var help: Boolean = false

    @Option(names = ["-y"], hidden = true)
    ...
    @Option(names = ["--experimental"], description = ["Enabled experimental rules (ktlint-ruleset-experimental)"])
